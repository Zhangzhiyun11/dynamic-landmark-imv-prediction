#!/usr/bin/env python3
"""V0.2 wrapper: deduplicate ID/predictor columns before outcome merge."""
...
base.MANIFEST_PATH = base.ROOT / "06_models" / "V2_GATE3_IMPLEMENTATION_MANIFEST_V0_2.json"


def load_and_verify_manifest() -> dict:
    manifest = json.loads(base.MANIFEST_PATH.read_text(encoding="utf-8"))
    expected_status = "FROZEN_AFTER_FAILED_MERGE_PREFLIGHT_BEFORE_FIRST_MODEL_FIT_OR_RESULT"
    if manifest["status"] != expected_status:
        raise RuntimeError("Gate 3 V0.2 manifest status is not frozen")
...
def load_dataset(database: str, model_id: str, manifest: dict):
    ...
    outcome = outcome.loc[
        outcome["outcome_imv"].isin([0, 1]), ["stay_key", "landmark_h", "outcome_imv"]
    ]

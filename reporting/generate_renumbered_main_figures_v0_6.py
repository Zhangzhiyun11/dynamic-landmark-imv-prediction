#!/usr/bin/env python3
"""Regenerate renumbered V0.6 main Figures 3 and 4 from locked aggregate CSV files.

The script deliberately reads aggregate outputs only. It writes editable SVG and
high-resolution PNG copies into the V0.6 source-assets directory without touching the
...
ROOT = Path(__file__).resolve().parents[4] / "study_b_v2"
RESULTS = ROOT / "07_results"
OUT = Path(__file__).resolve().parents[1] / "source_assets"
...
def figure2() -> None:
    ...
    im = Image.new("RGB", (width, height), WHITE)
    d = ImageDraw.Draw(im)
    d.text((80, 50), "Figure 3. Incremental AUPRC of dynamic versus snapshot CatBoost", font=F(50, True), fill=INK)
    ...
    svg = [svg_text(80, 90, "Figure 3. Incremental AUPRC of dynamic versus snapshot CatBoost", 50, weight=700),
    ...
    im.save(OUT / "V2_SR_FIGURE_3_SOURCE_V0_6.png", dpi=(300, 300))
    write_svg(OUT / "V2_SR_FIGURE_3_SOURCE_V0_6.svg", width, height, "\n".join(svg))
...
def figure3() -> None:
    ...
    im = Image.new("RGB", (width, height), WHITE)
    d = ImageDraw.Draw(im)
    d.text((80, 45), "Figure 4. Calibration transport and eICU alert-burden trade-off", font=F(50, True), fill=INK)
    ...
    svg = [svg_text(80, 82, "Figure 4. Calibration transport and eICU alert-burden trade-off", 50, weight=700),
    ...
    im.save(OUT / "V2_SR_FIGURE_4_SOURCE_V0_6.png", dpi=(300, 300))
    write_svg(OUT / "V2_SR_FIGURE_4_SOURCE_V0_6.svg", width, height, "\n".join(svg))


if __name__ == "__main__":
    figure2()
    figure3()
    for output in sorted(OUT.glob("V2_SR_FIGURE_[34]_SOURCE_V0_6.*")):
